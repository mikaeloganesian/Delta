...
class page():
    ...
    def createPage(self):
        ...
        title = Label(window, text=self.pageTitle, font=(
            'Times New Roman', '20'), fg='slate gray', pady=6)  # title creating
        title.pack()

        mainText = Text(window, font=('Arial', '12'),
                        width=46, height=14, wrap=WORD)
        mainText.insert(INSERT, self.mainText)
        mainText.configure(state='disabled')
        mainText.pack(padx=20)  # main text creating

        seeMore = Label(window, text='Смотреть подробнее:',
                        font=('Times New Roman', '14'), fg='gray')
        seeMoreUrl = Label(window, text=self.seeMoreUrl, font=(
